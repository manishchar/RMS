...
import alina.com.rms.activities.special_user_adptor.SpecialUserMasterViewListAdapter;
...
public class SpecialUserViewMastErictionList extends SpecialUserBaseActivity implements ServiceClickCallBack,AsyncCompleteListner,SwipeRefreshLayout.OnRefreshListener
{

    private RecyclerView mRecyclerView;
    private StaggeredGridLayoutManager mStaggeredLayoutManager;
    private SpecialUserMasterViewListAdapter servicesListAdapter;
    private List<MastProgressList> mastProgressLists=new ArrayList<MastProgressList>();
  //  RelativeLayout relativeLayout;
    ...
    protected void onCreate(Bundle savedInstanceState) {
     ...
     /*   String json = gson.toJson(login_response, type);
        ...
        login_response= LoginDB.getLoginResponseAsJSON(SpecialUserViewMastErictionList.this);
    ...
    private void hitToServer()
    {
        ...
        AsyncController asyncController=new AsyncController(SpecialUserViewMastErictionList.this,this,
                CallType.VIEW_MASTERICTION_MODEL,"",true);
        asyncController.setProgressDialoug(true);
        asyncController.getMastErictionProgressList();

    }

    private void initUI() {
        headingText.setText("Masteriction List");
        ...
        servicesListAdapter = new SpecialUserMasterViewListAdapter(SpecialUserViewMastErictionList.this, this,mastProgressLists);
    ...
    public void addHeadQuater()
    {
        ...
        Intent intent=new Intent(SpecialUserViewMastErictionList.this, SpecialUserMastErictionActivity.class);
        startActivity(intent);
    }


    public void updateHeadQuater( MastProgressList foundationProgressList)
    {
        Intent intent = new Intent(SpecialUserViewMastErictionList.this,SpecialUserMastErictionEditActivity.class);

        intent.putExtra("foundation_prog1__id",foundationProgressList.getMastProgId());
        startActivity(intent);
    ...
    public void deleteModule(final String id)
    {
        ...
        AlertDialog.Builder builder;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            builder = new AlertDialog.Builder(SpecialUserViewMastErictionList.this, android.R.style.Theme_Material_Dialog_Alert);
        } else {
            builder = new AlertDialog.Builder(SpecialUserViewMastErictionList.this);
        }
        builder.setTitle("Delete Public Module")
                .setMessage("Are you sure you want to delete this entry?")
                .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        // continue with delete
                        setProgressDialoug(true);
                        AsyncController asyncController=new AsyncController(SpecialUserViewMastErictionList.this, SpecialUserViewMastErictionList.this,CallType.DELETE_MODULE,"",true);
                        asyncController.setProgressDialoug(true);
                        asyncController.getDeleteViewMAstEriction(id);
                    }
                })
                .setNegativeButton(android.R.string.no, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
    ...
    public void asyncCompleteListner(String response, CallType callType) {
        ...
        if(response!=null)
        {
            if(callType==CallType.VIEW_MASTERICTION_MODEL)
            {
                ...
                if(response1!=null)
                {
                    if(response1.getMastProgressList()!=null)
                    {
                        Log.e("String","value");
                        mastProgressLists.clear();
                        mastProgressLists.addAll(response1.getMastProgressList());
                       // Log.e("value","".size());
                        servicesListAdapter.notifyDataSetChanged();

                    }
                    else {
                        Toast.makeText(SpecialUserViewMastErictionList.this,"no data found",Toast.LENGTH_LONG).show();
                    }
                }
                else {
                    Toast.makeText(SpecialUserViewMastErictionList.this,"no data found",Toast.LENGTH_LONG).show();
                ...
                if(response1!=null)
                {
                    if(response1.getResponseCode()!=null)
                    {
                        if(response1.getResponseCode()==1)
                        {
                            ...
                            Toast.makeText(SpecialUserViewMastErictionList.this, response1.getMessage(), Toast.LENGTH_LONG).show();
                            hitToServer();
                        }

                    }
                    else {
                        Toast.makeText(SpecialUserViewMastErictionList.this,"Headquater not added successfully",Toast.LENGTH_LONG).show();
                    }
                }
                else {
                    Toast.makeText(SpecialUserViewMastErictionList.this,"Headquater not added successfully",Toast.LENGTH_LONG).show();
            ...
            else if(callType==CallType.DELETE_MODULE)
            {
                ...
                if(response1!=null)
                {
                    if(response1.getResponseCode()!=null)
                    {
                        ...
                        Toast.makeText(SpecialUserViewMastErictionList.this,"Module not deleted successfully",Toast.LENGTH_LONG).show();
                    }
                }
                else {
                    Toast.makeText(SpecialUserViewMastErictionList.this,"Module not deleted successfully",Toast.LENGTH_LONG).show();
