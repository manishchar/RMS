...
import alina.com.rms.activities.special_user_adptor.SpecialUserViewWiringListAdapter;
...
public class SpecialUserViewWiringList extends SpecialUserBaseActivity implements ServiceClickCallBack,AsyncCompleteListner,SwipeRefreshLayout.OnRefreshListener
{

    private RecyclerView mRecyclerView;
    private StaggeredGridLayoutManager mStaggeredLayoutManager;
    private SpecialUserViewWiringListAdapter servicesListAdapter;
  ...
  //  RelativeLayout relativeLayout;
    ...
    protected void onCreate(Bundle savedInstanceState) {
     ...
     /*   String json = gson.toJson(login_response, type);
        ...
        login_response= LoginDB.getLoginResponseAsJSON(SpecialUserViewWiringList.this);
    ...
    private void hitToServer()
    {
        ...
        AsyncController asyncController=new AsyncController(SpecialUserViewWiringList.this,this,
                CallType.VIEW_MASTERICTION_MODEL,"",true);
        asyncController.setProgressDialoug(true);
        asyncController.getWiringProgressList();

    }

    private void initUI() {
        headingText.setText("Wiring Progress List");
        ...
        servicesListAdapter = new SpecialUserViewWiringListAdapter(SpecialUserViewWiringList.this, this,wiringProgressLists);
    ...
    public void addHeadQuater()
    {
        ...
        Intent intent=new Intent(SpecialUserViewWiringList.this, SpecialUserWiringActivity.class);
    ...
    {
        Intent intent = new Intent(SpecialUserViewWiringList.this,SpecialUserWiringEditActivity.class);
    ...
    public void deleteModule(final String id)
    {
        ...
        AlertDialog.Builder builder;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            builder = new AlertDialog.Builder(SpecialUserViewWiringList.this, android.R.style.Theme_Material_Dialog_Alert);
        } else {
            builder = new AlertDialog.Builder(SpecialUserViewWiringList.this);
        }
        builder.setTitle("Delete Public Module")
                .setMessage("Are you sure you want to delete this entry?")
                .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        // continue with delete
                        setProgressDialoug(true);
                        AsyncController asyncController=new AsyncController(SpecialUserViewWiringList.this, SpecialUserViewWiringList.this,CallType.DELETE_MODULE,"",true);
                        asyncController.setProgressDialoug(true);
                        asyncController.getDeleteViewWiring(id);
                    }
                })
                .setNegativeButton(android.R.string.no, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
    ...
    public void asyncCompleteListner(String response, CallType callType) {
        ...
        if(response!=null)
        {
            if(callType==CallType.VIEW_MASTERICTION_MODEL)
            {
                ...
                if(response1!=null)
                {
                    if(response1.getWiringProgressList()!=null)
                    {
                        Log.e("String","value");
                        wiringProgressLists.clear();
                        wiringProgressLists.addAll(response1.getWiringProgressList());
                       // Log.e("value","".size());
                        servicesListAdapter.notifyDataSetChanged();

                    }
                    else {
                        Toast.makeText(SpecialUserViewWiringList.this,"no data found",Toast.LENGTH_LONG).show();
                    }
                }
                else {
                    Toast.makeText(SpecialUserViewWiringList.this,"no data found",Toast.LENGTH_LONG).show();
                ...
                if(response1!=null)
                {
                    if(response1.getResponseCode()!=null)
                    {
                        if(response1.getResponseCode()==1)
                        {
                            ...
                            Toast.makeText(SpecialUserViewWiringList.this, response1.getMessage(), Toast.LENGTH_LONG).show();
                            hitToServer();
                        }

                    }
                    else {
                        Toast.makeText(SpecialUserViewWiringList.this,"Headquater not added successfully",Toast.LENGTH_LONG).show();
                    }
                }
                else {
                    Toast.makeText(SpecialUserViewWiringList.this,"Headquater not added successfully",Toast.LENGTH_LONG).show();
            ...
            else if(callType==CallType.DELETE_MODULE)
            {
                ...
                if(response1!=null)
                {
                    if(response1.getResponseCode()!=null)
                    {
                        ...
                        Toast.makeText(SpecialUserViewWiringList.this,"Module not deleted successfully",Toast.LENGTH_LONG).show();
                    }
                }
                else {
                    Toast.makeText(SpecialUserViewWiringList.this,"Module not deleted successfully",Toast.LENGTH_LONG).show();
